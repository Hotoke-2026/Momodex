...
export async function insertAchievement(userId: string, type: string, name: string): Promise<Achievement | null> {
  const id = crypto.randomUUID()
  
  await db.execute({
    sql: `INSERT INTO achievements (id, user_id, type, name, unlocked_at) 
  ...
  return (result.rows[0] as unknown as Achievement) || null
}
